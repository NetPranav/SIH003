...
| New Capability | Lives In | Why It Was Added |
|:---|:---|:---|
...
| IVR / Missed-Call Cognitive Line (zero-smartphone fallback) | **Phase 8** | Closes the "no device at all" accessibility gap in remote NER |
| Federated Learning for BKT/MMSE models | **Phase 5** | Strengthens the AI novelty story with privacy-preserving on-device learning |
...
| Policy Anchors (NPHCE, Rashtriya Vayoshri Yojana, e-Sanjeevani) + Cultural Cognitive Engagement Index | **Phase 12 & 18** | Converts funding/impact story into named, citable, government-aligned assets |
...
gantt
    ...
    section Foundation (P1-P3)
    Clinical & Cultural Research        :p1, 2026-01-01, 45d
    Design System & UX Architecture     :p2, after p1, 30d
    Infrastructure & Compliance Setup   :p3, after p1, 25d

    section Core Build (P4-P8)
    Patient PWA & Game Engine           :p4, after p2, 55d
...
|:---|:---|:---|
| Literature Review | LASI Wave-1, ARDSI Dementia India Report, WHO Global Dementia Action Plan, Cochrane RT meta-analyses (Woods et al.) | Annotated bibliography (50+ sources) |
| NER Dementia Prevalence Mapping | State-wise prevalence data across all 8 NER states | Prevalence heatmap document |
| Healthcare Infrastructure Audit | Neurologist density, PHC distribution, ASHA density per district | Infrastructure gap report |
| Connectivity Baseline Study | Cellular blackout zones, power outage frequency, broadband penetration (Majuli, Ri-Bhoi, Churachandpur) | Connectivity constraint matrix |

### Sub-Phase 1.2 — Neuropsychological Foundation
| Task | Details | Deliverable |
|:---|:---|:---|
| Ribot's Law Application Framework | How long-term episodic memory preservation will be leveraged vs. hippocampal decline | Clinical mechanism whitepaper |
| Reminiscence Therapy (RT) Protocol | Cochrane-aligned session structure (15–20 min, 2–3x/day) | RT Protocol v1.0 |
| MMSE/MoCA Proxy Correlation Model | Mapping telemetry vectors → cognitive assessment proxies | Proxy model specification |
| Anti-Agitation Design Principles | "Compassionate Interaction Design" rules — zero negative reinforcement | CID Principles Document |

### Sub-Phase 1.3 — Cultural Asset Research & Acquisition
| Task | Details | Deliverable |
|:---|:---|:---|
| Folk Instrument Cataloging | Pepa, Dhol, Pung, Duitara, Gogona, Tokari — tuning frequencies, overtones | Audio asset library |
| Indigenous Fauna Reference | Rhino, Hornbill, Red Panda, Sangai Deer, Hoolock Gibbon | Fauna visual reference pack |
| Traditional Textile Pattern Library | Muga, Gamosa, Mizo Puan, Naga Shawl motifs | Textile SVG/PNG library |
| Regional Language & Dialect Audit | 8 languages mapped with elder-relevant dialectal variation | Language matrix & phrase dictionary |

### Sub-Phase 1.4 — Life-Review & Storytelling Baseline (New)
| Task | Details | Deliverable |
|:---|:---|:---|
| Life-Review Therapy Literature Scan | Evidence base for narrative/legacy storytelling improving mood in dementia | Life-review evidence brief |
| Folklore & Oral History Sourcing | Partner with state archives/NGOs to source seed folklore per state | Seed folklore content pack |
| Elder Interview Pilot (Small n) | Record 10–15 sample life-story interviews to validate format/length | Sample recordings + format guide |

> [!IMPORTANT]
> **Milestone M1**: Clinical & Cultural Foundation Complete — peer-reviewed by 1 clinical neuropsychologist + 1 NER cultural consultant.
...
| Task | Details | Deliverable |
|:---|:---|:---|
| Color Palette | WCAG 2.2 AAA (≥7:1 contrast); warm, non-harsh tones | Design tokens (CSS variables) |
| Typography System | 24pt+ minimum; Assamese/Meitei/Bodo script rendering; Noto Sans fallback | Typography spec |
| Touch Target Specs | 64×64dp minimum, 16dp spacing, tremor-tolerant hitboxes | Touch target grid system |
| Iconography Library | High-contrast, culturally intuitive icons | Custom icon set (SVG) |
| Motion Guide | Gentle pulse/fade transitions, zero flicker | Motion design specification |

### Sub-Phase 2.2 — Information Architecture & Wireframes
| Task | Details | Deliverable |
|:---|:---|:---|
| Patient View IA | Max 3 nav cards/screen, zero nested menus | IA sitemap + flow diagram |
| Caregiver Portal IA | Dashboard-first: trajectory chart, adherence, reminder manager, wellness check-in | Caregiver wireframes |
| ASHA Worker Portal IA | Multi-patient cohort, Bluetooth sync, visit checklist | ASHA portal wireframes |
| Game & Social-Feature Wireframes | 3 games + Grandchild Connect + Community Circle screens | Wireframe set (20+ screens) |
| Reminder Flow Wireframes | Medicine card → voice → photo → confirm → log | Reminder flow wireframes |

### Sub-Phase 2.3 — High-Fidelity Prototyping
| Task | Details | Deliverable |
|:---|:---|:---|
| Patient View Hi-Fi Mockups | Home, games, reminders, memory album, social screens | Figma export |
| Caregiver Dashboard Hi-Fi | MMSE chart, adherence rings, wellness check-in panel | Figma export |
| Usability Testing (Remote) | 5–8 elderly + 5 caregivers via partner NGOs | Usability test report |
| Design Iteration | Contrast, button size, cultural relevance fixes | Hi-Fi prototypes v2.0 |

### Sub-Phase 2.4 — Accessibility & Zero-Device Interaction Design (New)
| Task | Details | Deliverable |
|:---|:---|:---|
| IVR Call-Flow Scripting | Voice-only menu tree for missed-call cognitive line | IVR flow diagram + scripts (8 languages) |
| Voice-Only Interaction Testing | Test comprehension of spoken menus with non-literate elders | IVR usability report |

> [!IMPORTANT]
> **Milestone M2**: Design System & Wireframes Approved — ≥85% task completion in usability tests; WCAG 2.2 AAA pass.
...
| Task | Details | Deliverable |
|:---|:---|:---|
| Repository Setup | Monorepo: `/client`, `/server`, `/ai-engine`, `/ivr-service`, `/docs`, `/assets` | Git repo + branch strategy |
| CI/CD Pipeline | Lint → test → build → deploy staging | Pipeline config |
| Code Quality Gates | ESLint, Prettier, Black, pre-commit hooks | Quality gate config |
| Development Standards Document | Conventions, commit format, PR review process | Developer handbook |

### Sub-Phase 3.2 — Cloud Infrastructure Provisioning
| Task | Details | Deliverable |
|:---|:---|:---|
| Cloud Provider Selection | AWS/GCP/Azure India region; gov cloud compliance | Provider selection doc |
| Server Architecture | FastAPI cluster + TimescaleDB + Redis + Celery | Infrastructure-as-Code |
| SSL/TLS Configuration | TLS 1.3, certificate pinning, DISHA-compliant encryption | Security configuration |
| Staging Environment | Synthetic patient data mirror | Staging URL + credentials |

### Sub-Phase 3.3 — Security & Compliance Framework
| Task | Details | Deliverable |
|:---|:---|:---|
| DISHA Compliance Checklist | Map DISHA requirements to system components | Compliance matrix |
| ABDM Integration Plan | ABHA ID linking workflow; sandbox credentials | ABDM integration spec |
| Data Classification Policy | PHI vs. behavioral telemetry segregation; SHA-256 pseudo-IDs | Data classification document |
| Threat Model | STRIDE analysis incl. edge device, BLE relay, IVR gateway, cloud | Threat model report |

### Sub-Phase 3.4 — Telephony & IVR Infrastructure (New)
| Task | Details | Deliverable |
|:---|:---|:---|
| Toll-Free Number Provisioning | Partner with BSNL/telecom provider for NER-wide toll-free access | Provisioned number + SLA |
| IVR Platform Selection | Evaluate Exotel/Knowlarity/Asterisk for multilingual IVR at scale | Platform selection doc |
| Call Data Security | Encrypt call logs; link to pseudo-ID, not raw phone numbers where feasible | Call data security spec |

### Sub-Phase 3.5 — Federated Learning Infrastructure Groundwork (New)
| Task | Details | Deliverable |
|:---|:---|:---|
| FL Framework Evaluation | Assess Flower / TensorFlow Federated for edge-compatible aggregation | FL framework selection doc |
| Aggregation Server Design | Design server-side model-averaging service (no raw data ingestion) | FL aggregation server spec |

> [!IMPORTANT]
> **Milestone M3**: Infrastructure Ready — CI/CD operational, staging accessible, IVR number provisioned, threat model reviewed.
...
| Task | Details | Deliverable |
|:---|:---|:---|
| PWA Scaffolding | Service Worker, Web App Manifest, cache-first/network-first strategy | PWA boilerplate |
| App Shell Architecture | Skeleton → paint; 3-card bottom nav | App shell implementation |
| Design System Implementation | Component library (Button, Card, Modal, Toast, Chart) | Component library |
| Responsive Layout Engine | 320px–1024px fluid grid; orientation lock for games | Responsive CSS framework |
| Accessibility Layer | ARIA labels, focus management, screen reader, haptics | Accessibility middleware |
...
> [!IMPORTANT]
> **Milestone M4**: Patient PWA + all 4 games playable with AACB — Lighthouse PWA ≥95, Accessibility =100, zero negative audio cues.
...
> [!IMPORTANT]
> **Milestone M5**: DCDA + Federated Learning Operational — BKT <5ms; MMSE proxy r≥0.75; federated updates verified privacy-safe.
...
| Task | Details | Deliverable |
|:---|:---|:---|
| Voice Recording Interface | Guided flow with quality meter | Voice recorder component |
| Audio Processing Pipeline | Normalize, trim, compress to Opus, encrypt | Audio processing module |
| Clip Playback Engine | Trigger clips at reminder times with photo | Playback integration |
| Voice Clip Management | Preview, re-record, delete, up to 10 clips | Voice clip CRUD interface |
...
> [!IMPORTANT]
> **Milestone M6**: Multilingual Voice System Operational — TTS in all 8 languages; ASR <500ms; weekly NLG summaries generated correctly.
...
> [!IMPORTANT]
> **Milestone M7**: Social Layer Functional — Grandchild Connect round-trip works offline-to-sync; ≥1 pilot Community Circle run successfully; 10+ legacy stories captured in test cohort.
...
> [!IMPORTANT]
> **Milestone M8**: IVR Line Operational — calls connect reliably across 3 telecom circles in NER; check-in scoring and reminder delivery verified end-to-end.
...
> [!IMPORTANT]
> **Milestone M9**: All Dashboard Views Functional — MMSE chart renders on synthetic data; BLE sync <30s for 2 weeks telemetry; wellness check-in live.
...
> [!IMPORTANT]
> **Milestone M10**: Reminder System End-to-End Functional — fires within ±30s; voice auto-plays; adherence logs persist offline across both channels.
...
> [!IMPORTANT]
> **Milestone M11**: Offline-First & Safety Mesh Verified — 30+ days offline with zero data loss; BLE relay <30s for 2 weeks of data; zone-exit alerts fire correctly in field test.
...
> [!IMPORTANT]
> **Milestone M12**: Government Integration Complete — all API endpoints pass integration tests; ABHA links in sandbox; e-Sanjeevani referral test succeeds; scheme citations verified current.
...
> [!IMPORTANT]
> **Milestone M13**: QA & Compliance Gates Passed — 90%+ coverage; Accessibility=100; zero critical vulnerabilities; social/IVR features verified.
...
> [!IMPORTANT]
> **Milestone M14**: Clinical Pilot Complete — ≥70% daily engagement; MMSE proxy r≥0.70; adherence ≥85%; zero critical adverse events; caregiver satisfaction ≥4.0/5.0.
...
> [!IMPORTANT]
> **Milestone M15**: Post-Pilot v2.0 Ready — critical bugs resolved; models recalibrated; MMSE proxy correlation improved to r≥0.80; CCEI drafted.
...
| Wave | States | Target PHCs | Target Patients | Timeline |
|:---|:---|:---|:---|:---|
...
> [!IMPORTANT]
> **Milestone M16**: Multi-State Rollout on Track — Wave 1–4 PHCs onboarded per schedule; state locale packs validated.
...
> [!IMPORTANT]
> **Milestone M17**: 1,500+ ASHA Workers Trained — including Circle facilitation and IVR support certification.
...
> [!IMPORTANT]
> **Milestone M18**: Central Hub & CCEI Operational — active in all 8 states; 5,000+ enrolled patients; 1,500+ trained ASHAs; CCEI live on all dashboards.
...
> [!IMPORTANT]
> **Milestone M19**: Pan-NER Public Launch Complete — available on Play Store, PWA, and public IVR line across all 8 states.
...
> [!IMPORTANT]
> **Milestone M20**: Sustainability Framework Operational — ≥50,000 registered patients in first year; sustainable funding secured; governance, ethics, and community structures self-running.
...
timeline
    ...
    Month 1-2   : P1 Clinical & Cultural Research
                : P2 Design System & UX
                : P3 Infrastructure & Compliance
    Month 3-6   : P4 Patient PWA & Games
...
> [!TIP]
> **Quick Reference — Deliverable Count by Phase Group**
> - **Foundation (P1–P3)**: ~45 deliverables
> - **Core Build (P4–P8)**: ~65 deliverables (incl. social + IVR)
> - **Ecosystem (P9–P13)**: ~50 deliverables (incl. BLE mesh + wellness)
> - **Pilot & Iterate (P14–P15)**: ~30 deliverables
> - **Scale (P16–P18)**: ~30 deliverables (incl. CCEI)
> - **Sustain (P19–P20)**: ~20 deliverables
